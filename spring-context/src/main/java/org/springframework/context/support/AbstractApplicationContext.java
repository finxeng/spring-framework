...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				 ...
				 * 将所有实现了 BeanPostProcessor 接口的类加载到 BeanFactory的beanPostProcessors属性中。
				 *
				...
				// 初始话applicationEventMulticaster 且注册为bean
				...
				// 时间监听器的创建，多播器(个人理解其实就是listener的集合处理拓展类)
				registerListeners();

				// Instantiate all remaining (non-lazy-init) singletons.
				/**
				 * Bean首先实例化、再初始化
				 * BeanPostProcessor的所有实现类的前后置处理器会依次在bean的初始化前后执行
				 */
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		/**
		 * 注意spring项目其实是创建了一个XmlWebApplicationContext，根据类继承关系，refreshBeanFactory()、getBeanFactory()
		 * 实际执行的是AbstractRefreshableApplicationContext类中的方法
		 */
		// 创建DefaultListableBeanFactory，且根据xml文件创建beanDefinitions
		refreshBeanFactory();
		// 将上一步创建的DefaultListableBeanFactory 返回
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * 跳过如下类的自动装配，因为在上一步ApplicationContextAwareProcessor中已经完成了手动注入
		 * 例如，如果不使用注解的话，可以在需要应用上下文的类实现ApplicationContextAware接口即可获取到应用上下文
		 */
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		//获取到当前应用上下文中的beanFactoryPostProcessors
		//之后实例化这些bean，且显示的调用
